...
//   float mouseYT = mouseY + time * 0.15;
  ...
  // float fullScale = 500.0;
  // float scale = resolution.y / fullScale;
  // float scaleInv = fullScale / resolution.y;
  ...
  vec2 cube1 = roundBox(position, vec3(2.0), 0.5, vec3(xBase - 2.5, 7.0, 0.0), quat);
  // vec2 cube2 = roundBox(position, vec3(1.5), 0.5, vec3(xBase, 0.0, 0.0), quat);
  vec2 cube3 = roundBox(position, vec3(2.0), 0.5, vec3(xBase + 2.5, -7.0, 0.0), quat);
